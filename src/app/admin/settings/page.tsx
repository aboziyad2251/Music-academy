...
export default function AdminSettingsPage() {
  ...
  const toggles = [
    {
      key: "enable_ai_chat" as const,
      label: "AI Chat Assistant",
      desc: "Allow students to use AI-powered chat for lesson help.",
      icon: MessageSquare,
      color: "text-indigo-400",
    },
    {
      key: "enable_registrations" as const,
    ...
    },
    {
      ...
      desc: "Display a maintenance page to all non-admin users.",
      icon: Wrench,
    ...
    <div className="space-y-8 max-w-2xl pb-6">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-1 text-sm">
          Configure global platform behavior and feature flags.
        </p>
      </div>
      ...
      <div className="rounded-xl bg-slate-900 border border-slate-800 p-6 space-y-4">
        ...
        <div className="space-y-2">
          {toggles.map(({ key, label, desc, icon: Icon, color }) => (
            <label
              key={key}
              className="flex items-center justify-between p-4 rounded-xl border border-slate-800 hover:bg-slate-800/50 cursor-pointer transition-colors group"
            >
              ...
              <div className="flex-shrink-0 ml-4">
                <div
                  className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${
                    settings[key] ? "bg-amber-600" : "bg-slate-700"
                  }`}
                >
                  <div
      ...
      <div className="rounded-xl bg-slate-900 border border-slate-800 p-6 space-y-4">
        ...
        <div className="flex items-center gap-4 p-4 rounded-xl border border-slate-800 bg-slate-800/30">
          ...
          <Badge className="bg-amber-900/60 text-amber-400 border border-amber-800/40">
            admin
          </Badge>
        </div>
      </div>
      ...
      <Button
        onClick={handleSave}
        disabled={saving}
        className="w-full bg-amber-600 hover:bg-amber-700 text-white h-11 text-sm font-semibold"
      >
        {saving ? (
