...
export default function CreateCoursePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (file) {
        ...
        if (uploadError) {
          throw new Error(`Failed to upload thumbnail. Make sure a public 'course-content' bucket exists. (${uploadError.message})`);
        }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-6">
      <div>
        <Link href="/teacher/courses" className="text-sm text-slate-500 hover:text-slate-300 mb-4 inline-block transition-colors">
          &larr; Back to Courses
        </Link>
        <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">Create New Course</h1>
        <p className="text-slate-400 mt-1 text-sm">Start building your new music course here.</p>
      </div>
